...
that might be accessed by multiple functions 
during the request.
...
current_app is another special object 
that points to the Flask application handling the request. 
...
has been created and is handling a request, s
o current_app can be used.
'''
def get_db():
...
# that command called init-db that calls the init_db function 
